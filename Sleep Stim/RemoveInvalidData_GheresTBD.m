function [decData,decFileIDs,decDurations,decEventTimes] = RemoveInvalidData_GheresTBD(data,fileIDs,durations,eventTimes,ManualDecisions)
...
for a = 1:size(data,1)
    ...
    for b = 1:length(ManualDecisions.fileIDs)
        [~,~,manualFileID] = GetFileInfo_GheresTBD(ManualDecisions.fileIDs{b,1});
